...
begin
 ...
 delimiter//
create procedure pa_autor_totalypromedio(in p_autor varchar(30), out suma decimal(6,2), out promedio decimal(6,2))
begin
	select titulo, editorial, precio from libros where autor=p_autor;
    select sum(precio) as suma from libros where autor=p_autor;
    select avg(precio) as promedio from libros where autor=p_autor;
end
// delimiter
