...
select  editoriales.nombre,codigoeditorial from libros join editoriales on libros.codigoeditorial=editoriales.codigo where autor='Richard Bach'; 
 ...
 # Esta query indica si existen algunos libros del autor borges que fueron escritos bajo la misma editorial del autor richard bach
 select titulo from libros where autor='Borges' and codigoeditorial= any
 (select e.codigo from editoriales as e join libros as l on e.codigo=l.codigoeditorial where l.autor='Richard Bach');
 /* OJO con la query anterior por que si o se utilizan alias para las tablas se genera una ambiguedad*/
 select titulo from libros where autor='Borges' and codigoeditorial = any 
 (select e.codigo from editoriales as e join libros as l on codigoeditorial=e.codigo where l.autor='Richard Bach');
 
 #todas las editoriales que publican libros de borges publican tambien libros de bach
 select titulo from libros where autor='Borges' and codigoeditorial=all
 (select e.codigo from editoriales as e join libros as l on codigoeditorial=l.codigo where l.autor='Richard Bach');
 select titulo from libros where autor='Borges' and codigoeditorial = all
 (select e.codigo from editoriales as e join libros as l on codigoeditorial=e.codigo where l.autor='Richard Bach');
 
 #Selecciona todos los libros de borges que tengan un mayor precio que cualquier libro de bach
 select precio, titulo from libros where autor='Borges' and precio > any
 (select precio from libros where autor='Richard Bach');
 select titulo, precio from libros where autor='Borges' and precio > any
 (select precio from libros where autor = 'Richard Bach');
 
 #Todos los libros de borges que son  mas caros que los de bach con su nombre y editorial 
 select titulo, precio, nombre from libros as l  join editoriales as e on autor= 'Borges' and l.codigoeditorial=e.codigo  where precio > all
 (select precio from libros where autor= 'Richard Bach');
...
drop table if exists detalles;
 ...
 # Lista de clientes que compraron un articulo llamado lapiz.
 select cliente, numero from facturas as f where exists
 (select * from detalles as d where d.numerofactura=f.numero and articulo= 'lapiz');
 select cliente, numero from facturas as f where exists
 (select * from detalles as d where f.numero=d.numerofactura and d.articulo='lapiz');
#Lista a los clientes quienes no han comprado un articulo llamado lapiz.
...
(select * from detalles as d where f.numero=d.numerofactura and d.articulo='lapiz'); 
 ...
 insert into libros(titulo,autor,codigoeditorial,precio)
   ...
   select * from libros;
   #Subconsulta de ejemplo de actualización: actualizar el precio de la editorial emece 
   update libros set precio=precio+(0.1*precio) where codigoeditorial = (select codigo from editoriales where nombre='emece');
   ...
   #Subconsulta que elimina los libros que tengan como editorial, la editorial planeta
